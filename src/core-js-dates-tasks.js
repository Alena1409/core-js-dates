...
function getDayName(date) {
  ...
  const objDate = new Date(date);
  return arrayDay[objDate.getUTCDay()];
}
...
function getNextFriday(date) {
  ...
  for (let i = 1; i <= 7; i += 1) {
    ...
    newDate.setDate(copyDate.getDate() + i);
    if (arrayDay[newDate.getDay()] === 'Friday') {
      return newDate;
    }
  }
  return copyDate;
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const timestampDateStart = new Date(dateStart).getTime();
  const timestampDateEnd = new Date(dateEnd).getTime();
  const millisecondsInDay = 24 * 60 * 60 * 1000;
  const differenceInTimestamp = timestampDateEnd - timestampDateStart;
  return differenceInTimestamp / millisecondsInDay + 1;
}
...
function isDateInPeriod(date, period) {
  const timestampStart = new Date(period.start);
  const timestampEnd = new Date(period.end);
  const timestampDate = new Date(date);
  return timestampDate >= timestampStart && timestampDate <= timestampEnd;
}
...
function formatDate(date) {
  ...
  const year = objData.getUTCFullYear();
  const month = objData.getUTCMonth() + 1;
  const dates = objData.getUTCDate();
...
function getWeekNumberByDate(date) {
  ...
  const differenceTime = date - startOfYear;
  const millisecondsInDay = 24 * 60 * 60 * 1000;
  const countDay = differenceTime / millisecondsInDay + 1;
  return Math.ceil(countDay / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startDate = period.start.split('-').reverse().join('-');
  const endDate = period.end.split('-').reverse().join('-');
  ...
  while (currentDate <= end) {
    for (let i = 0; i < countWorkDays; i += 1) {
      if (currentDate <= end) {
        arrayWorksDay.push(new Date(currentDate));
        currentDate = new Date(currentDate.getTime() + millisecondsInDay);
      }
    }
    currentDate = new Date(
      currentDate.getTime() + millisecondsInDay * countOffDays
    );
  }
  const resultArray = arrayWorksDay.map((value) => {
